...
text_data=bruceR::import(here::here('data','penguin','penguin_rawdata_new.csv'))
...
x <- c('男','女','女','男')
fx=factor(x)
df=data.frame(c(fx),c(97,99,95,96),c('A','A','B','B'),c('T','F','T','F'))
df
class(df[[1]])
```
...
letter <- c('A','A','B','B')
logic <- c('T','F','T','F')
data <- data.frame(gender1,score,letter,logic)
data
class(data[[1]])
